...
function sortArr(comparator, array) {
  var place; //need a placeholder variable for later 
  var new_arr = array.slice(); //use .slice() to copy that array into the variable 'new_arr' to return later
  for (var i = 0; i < new_arr.length-1; i++) 
  {
      for (var next=i+1; next < new_arr.length; next++)
      {
          if (comparator(new_arr[next], new_arr[i]))
          {
              place = new_arr[i];
              new_arr[i] = new_arr[next];
              new_arr[next] = place;
          }
      }
  }
...
function clubComparator(student1, student2) {
  var comparison = function(student) {
      switch(student.club.toUpperCase()) //Simple way to assign a number to the various different clubs 
      {
          case "IMPROV":
          return 5;
          break;
  ...
  if (comparison(student1) == comparison(student2)) {
      return yearComparator(student1, student2);
  }
  
  else if (comparison(student1) > comparison(student2)) {
      return true;
  }
...
display = function(students, type) {
    for (var person in students) {
        students[person].logMe(type)
    }
}
...
display(sortArr(yearComparator, students), false);

console.log("**********");
console.log("The students sorted by major are: "); 
display(sortArr(majorComparator, students), false);

console.log("**********");
console.log("The students sorted by club affiliation are: "); 
display(sortArr(clubComparator, students), true);
